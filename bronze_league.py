...
def targetable_area(area, Enemies, players):
    ...
    for agent in Players:
        if (agent.x, agent.y) in area:
            return False
    print(area, file=sys.stderr, flush=True)
    for enemy_agent in Enemies:
...
def select_area_to_target(list_area, players, Enemies):
    for area in list_area:
        if targetable_area(area, Enemies, players):
            return area
...
def brute_force(list_area, area_to_target):
    for i in range(len(list_area)):
        if list_area[i] == area_to_target:
            return  -4 if i % 2 == 0 else 4
...
right_area = [
    (width-4, 1), (width-3, 1), (width-2, 1),
    (width-4, 2), (width-3, 2) , (width-2, 2),
    (width-4, 3), (width-3, 3), (width-2, 3)
]
...
se_area = [
    (width-4, 8), (width-3, 8), (width-2, 8),
    (width-4, 9), (width-3, 9), (width-2, 9),
    (width-4, 10), (width-3, 10), (width-2, 10)
]
pos_x = 0
list_area = [left_area, right_area, so_area, se_area]
left = (6, 2)
right = (width-7, 2)
print(width-7, file=sys.stderr, flush=True)
bot_left = (6, 9)
bot_right = (width-7, 9)
map_area = {left: left_area, right: right_area, bot_left:so_area, bot_right:se_area}
while True:
    Players.clear()
    Enemies.clear()
    ...
    if Enemies != None:
        for agent in Players:
            if agent.x == width//2 and agent.y == height//2:
                my_a = agent.agent_id
            if my_a == agent.agent_id:
                area_to_target = select_area_to_target(list_area, Players, Enemies)
                print(get_key(map_area, area_to_target),  file=sys.stderr, flush=True)
                if not has_reach_target(agent, get_key(map_area, area_to_target)):
                    new_x, new_y = move_to_the_target(agent, get_key(map_area, area_to_target))
                    print(f"{agent.agent_id};MOVE {new_x} {new_y}")
                else:
                    if area_to_target is not None:
                        pos_x = agent.x + brute_force(list_area, area_to_target)
                        print("debug", file=sys.stderr, flush=True)
                        print(f"{agent.agent_id};THROW {pos_x} {agent.y}")
            else:
